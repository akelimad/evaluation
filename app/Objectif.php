...
class Objectif extends Model
{
  ...
  public static function getTotalNote($eid, $user_id, $objectif_id, $role = 'user')
  {
    $total = 0;
    ...
    if (!empty($data)) {
      foreach ($data as $indicator) {
        if (!isset($indicator['ponderation'])) continue;
        $realizedPercent = (Objectif_user::getRealised($eid, $user_id, $objectif_id, $indicator['id'], $role) / $indicator['fixed']) * 100;
        $total += $realizedPercent * ($indicator['ponderation'] / 100);
      }
      $total = round($total);
    }
