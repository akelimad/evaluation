      ...
      <div class="col-md-12">
        ...
        <div class="box box-primary">
          <div class="filter-box mb40">
            ...
            <form action="{{ url('entretiens/evaluations') }}" class="criteresForm"
                  style="{{ $params ? 'display: block;':'' }}">
              <div class="row">
                <div class="col-md-3">
                  <div class="form-group">
                    <label for="datepicker">Date d'expiration</label>
                    <input type="text" name="dlimite" id="datepicker" class="form-control"
                           value="{{ isset($dlimite) && !empty($dlimite) ? Carbon\Carbon::parse($dlimite)->format('d-m-Y') :'' }}"
                  ...
                  <div class="form-group">
                    ...
                    <select name="title" id="title" class="form-control">
                      ...
                      @foreach($entretiens as $e)
                        <option
                            value="{{ $e->id }}" {{ isset($title) && $title == $e->id ? 'selected':'' }}>{{ $e->titre }}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
              ...
              <div class="row">
                <div class="col-md-12">
                  ...
                  <a href="{{url('entretiens/evaluations')}}" class="btn btn-default"><i class="fa fa-refresh"></i>
                    Actualiser</a>
                </div>
              </div>
            </form>
              ...
              <form action="{{ url('notifyMentorsInterview') }}" method="POST">
                ...
                <table class="table table-hover table-striped table-inversed-blue">
                  ...
                  @foreach($results as  $k => $row)
                    <tr class="{{ App\User::hasMotif($row->entretienId, $row->userId) ? 'has-motif': 'no-motif' }}"
                        data-toggle="tooltip"
                        title="{{ App\User::hasMotif($row->entretienId, $row->userId) ? 'Il ya un motif mentionné pour '.$row->name.' '.$row->last_name.'. cliquer sur l\'icon de paramétrage pour le mettre à jour' : '' }}">
                      @if(App\Entretien::countUnanswered()>0)
                        <td>
                          @if(!App\Entretien::answeredMentor($row->entretienId, $row->userId, App\User::getMentor($row->userId) ? App\User::getMentor($row->userId)->id : $row->userId))
                            <div class="wrap-checkItem">
                              <input type="checkbox" name="data[{{$k}}][mentorId]" class="usersId checkItem"
                                     autocomplete="off"
                                     value="{{ App\User::getMentor($row->userId) ? App\User::getMentor($row->userId)->id: $row->userId }}">
                      ...
                      <td class="text-center">
                        <span
                            class="label label-{{App\Entretien::answered($row->entretienId, $row->userId) == true ? 'success':'danger'}} empty"
                            data-toggle="tooltip"
                            title="{{App\Entretien::answered($e->id, $row->userId) ? 'Remplie le '.Carbon\Carbon::parse(App\Entretien::answered($e->id, $row->userId)->user_updated_at)->format('d/m/Y à H:i') : ''}}"> </span>
                      </td>
                      <td class="text-center">
                        <span
                            class="label label-{{App\Entretien::answeredMentor($row->entretienId, $row->userId, App\User::getMentor($row->userId) ? App\User::getMentor($row->userId)->id : $row->userId ) ? 'success':'danger'}} empty"
                            data-toggle="tooltip"
                            title="{{App\Entretien::answeredMentor($e->id, $row->userId, App\User::getMentor($row->userId)->id) ? 'Validée par mentor le '.Carbon\Carbon::parse(App\Entretien::answered($e->id, $row->userId)->mentor_updated_at)->format('d/m/Y H:i') :''}}"> </span>
                      </td>
                      <td class="text-center">
                      ...
                      <td class="text-center">
                        @if (App\Entretien::notationBySectionOrItem($row->entretienId))
                          ...
                          <span
                              class="badge">{{ $row->note > 0 ? App\Answer::formated($row->note)."/$count" : "0/$count" }}</span>
                        @endif
                      </td>
                      <td class="text-center">
                        @if(!App\Entretien::answeredMentor($row->entretienId, $row->userId, App\User::getMentor($row->userId) ? App\User::getMentor($row->userId)->id : $row->userId))
                        ...
                        <div class="">
                          <div class="col-md-6">
                            <select name="motif" id="motif-row-{{$row->userId}}" class="form-control">
                              <option value=""> Veuillez indiquer le motif d'abscence
                                pour {{ $row->name." ".$row->last_name }} </option>
                              <option
                                  value="MAL" {{App\User::hasMotif($row->entretienId, $row->userId) == "MAL" ? 'selected' : ''}} >
                                Abscence maladie sur la periode
                              </option>
                              <option
                                  value="MAT" {{App\User::hasMotif($row->entretienId, $row->userId) == "MAT" ? 'selected' : ''}} >
                                Abscence maternité sur la periode
                              </option>
                              <option
                                  value="CP" {{App\User::hasMotif($row->entretienId, $row->userId) == "CP" ? 'selected' : ''}}>
                                Abscence congé parental sur la periode
                              </option>
                              <option
                                  value="INV" {{App\User::hasMotif($row->entretienId, $row->userId) == "INV" ? 'selected' : ''}} >
                                Abscence invalidité sur la periode
                              </option>
                              <option
                                  value="CIP" {{App\User::hasMotif($row->entretienId, $row->userId) == "CIP" ? 'selected' : ''}} >
                                Abscence pour congé individuel de la formation sur la periode
                              </option>
                              <option
                                  value="FPI" {{App\User::hasMotif($row->entretienId, $row->userId) == "FPI" ? 'selected' : ''}} >
                                Entretien de fin de période d'essai prévu sur la periode
                              </option>
                              <option
                                  value="AD" {{App\User::hasMotif($row->entretienId, $row->userId) == "AD" ? 'selected' : ''}} >
                                Annulation de la demande d'entretien
                              </option>
                              <option
                                  value="SE" {{App\User::hasMotif($row->entretienId, $row->userId) == "SE" ? 'selected' : ''}} >
                                Sortie des effectifs
                              </option>
                              <option
                                  value="AUTRE" {{App\User::hasMotif($row->entretienId, $row->userId) == "AUTRE" ? 'selected' : ''}} >
                                Autre motif
                              </option>
                            </select>
                          </div>
                          <div class="col-md-6">
                            <button type="button" class="btn btn-success motifUpdateBtn"
                                    data-entretien-id="{{$row->entretienId}}" data-user-id="{{$row->userId}}"><i
                                  class="fa fa-check"></i> Sauvegarder
                            </button>
                          </div>
                ...
                <div class="sendInvitationBtn mb40">
                  <button type="submit" class="btn btn-success"><i class="fa fa-envelope"></i> Relancer le(s) mentor(s)
                  </button>
                </div>
              </form>
            </div>
