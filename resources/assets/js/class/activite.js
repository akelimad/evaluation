...
export default class chmActivite {

  static create () {
    var eid = $('.addBtn').data('id')
    console.log(eid)
    window.chmModal.show({type: 'GET', url: window.chmSite.url('entretiens/' + eid + '/activites/create')}, {
      form: {
        class: 'allInputsFormValidation form-horizontal',
        callback: 'chmActivite.store'
      },
      footer: {
        label: 'Sauvegarder'
      },
  ...
  static edit (params) {
    window.chmModal.show({type: 'GET', url: window.chmSite.url('entretiens/' + params.e_id + '/activites/' + params.id + '/edit')}, {
      form: {
        class: 'allInputsFormValidation form-horizontal',
        callback: 'chmActivite.store'
      },
  ...
  static store (event) {
    ...
    var ajaxParams = {
      id: id,
      type: 'POST',
      url: window.chmSite.url('entretiens/' + eid + '/activites/store'),
      data: data,
      processData: false,
      contentType: false,
      cache: false,
