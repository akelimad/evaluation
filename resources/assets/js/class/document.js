...
export default class chmDocument {

  static create () {
    var eid = $('.addBtn').data('id')
    window.chmModal.show({type: 'GET', url: window.chmSite.url('entretiens/' + eid + '/documents/create')}, {
      form: {
        class: 'allInputsFormValidation form-horizontal',
        callback: 'chmDocument.store'
      },
      footer: {
        label: 'Sauvegarder'
      }
  ...
  static edit (params) {
    window.chmModal.show({type: 'GET', url: window.chmSite.url('entretiens/' + params.e_id + '/documents/' + params.id + '/edit')}, {
      form: {
        class: 'allInputsFormValidation form-horizontal',
        callback: 'chmDocument.store'
      },
  ...
  static store (event) {
    ...
    var ajaxParams = {
      id: id,
      type: 'POST',
      url: window.chmSite.url('entretiens/' + eid + '/documents/store'),
      data: data,
      processData: false,
      contentType: false,
      cache: false,
