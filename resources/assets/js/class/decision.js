...
export default class chmDecision {

  static create () {
    var eid = $('.addBtn').data('id')
    window.chmModal.show({type: 'GET', url: window.chmSite.url('entretiens/' + eid + '/decisions/create')}, {
      form: {
        class: 'allInputsFormValidation form-horizontal',
        callback: 'chmDecision.store'
      },
      footer: {
        label: 'Sauvegarder'
      }
  ...
  static edit (params) {
    window.chmModal.show({type: 'GET', url: window.chmSite.url('entretiens/' + params.e_id + '/decisions/' + params.id + '/edit')}, {
      form: {
        class: 'allInputsFormValidation form-horizontal',
        callback: 'chmDecision.store'
      },
  ...
  static store (event) {
    ...
    var ajaxParams = {
      id: id,
      type: 'POST',
      url: window.chmSite.url('entretiens/' + eid + '/decisions/store'),
      data: data,
      processData: false,
      contentType: false,
      cache: false,
